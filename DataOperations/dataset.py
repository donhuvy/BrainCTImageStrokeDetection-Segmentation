...
import imutils
import random
from albumentations import CenterCrop, RandomRotate90, GridDistortion, HorizontalFlip, VerticalFlip, Crop, \
...
class Dataset:
    ...
    def importImages(self):
        ...
        trainingArrayX, testArrayX, trainingArrayY, testArrayY = np.array(trainingListX), np.array(testListX), np.array(
            trainingListY), np.array(testListY)
        ...
        for i in range(len(self.pathList)):
            for subdir, dirs, files in os.walk(self.pathList[i]):
                ...
                for file in files:
                    ...
                    filestr = file
                    filestr2 = file
                    if len(filestr.split(".")) == 2 and filestr.split(".")[-1] in ['png', 'pjg', 'jpeg'] and len(
                            filestr2.split(" ")) == 1:
                        splitter = filepath
                    ...
                    counterin += 1
        trainingArrayX, testArrayX, trainingArrayY, testArrayY, nameArray = np.array(trainingListX), np.array(
            testListX), np.array(trainingListY), np.array(testListY), np.array(nameList)
        shuffler = np.random.permutation(len(trainingArrayX))
        self.trainingArrayX, self.trainingArrayY = trainingArrayX[shuffler], trainingArrayY[shuffler]
        shufflerTest = np.random.permutation(len(testArrayX))
        self.testArrayX, self.testArrayY = testArrayX[shufflerTest], testArrayY[shufflerTest]
    ...
    def getInfo(self):
        ...
        Image properties are:
            Size: {}
            Data type: {}
        """.format(len(self.trainingArrayX), len(self.testArrayX), len(self.pathList) / 2, self.trainingArrayX[0].shape,
                   self.dataType))
